...
function validateSkincareProduct(product) {
  ...
  if (!Array.isArray(product.images) || !product.images.some((image) => typeof image === "string" && image.trim())) return { eligible: false, reason: "MISSING_IMAGE" };
  if (!Number.isFinite(Number(product.price)) || Number(product.price) <= 0) return { eligible: false, reason: "INVALID_PRICE" };
  if (product.status !== "Active" || metadata.active === false) return { eligible: false, reason: "INACTIVE" };
  if (Number(product.stock) <= 0 || metadata.inStock === false) return { eligible: false, reason: "OUT_OF_STOCK" };
...
function scoreProduct(product, profile, slot) {
  ...
  return {
    ...
    score,
    reasons: [
      ...primary.map((need) => `Supports ${need.replace(/_/g, " ")}`),
      ...(profile.skinType && metadata.skinTypes?.includes(profile.skinType) ? [`Suitable for ${profile.skinType} skin`] : []),
...
function recommendProductsForSkinProfile(profileInput, products = []) {
  ...
  const routine = routineSlots(profile).map((slot) => {
    ...
    return { ...slot, recommended: matches[0] || null, alternatives: matches.slice(1, 4) };
  });
