...
const externalSkincareCatalogPath = path.join(root, "data", "skincare-product-import.india-verified.json");
...
function externalSkincareProducts() {
  try {
    const catalog = JSON.parse(fs.readFileSync(externalSkincareCatalogPath, "utf8"));
...
function mergeCatalogProducts(products) {
  const byId = new Map();
  [...(products || []), ...externalSkincareProducts()].forEach((product) => {
...
function emptyStore() {
  return {
    products: mergeCatalogProducts(defaultProducts()),
    settings: {
      storeName: "Dearelle",
...
function normalizeStore(store) {
  ...
  return {
    products: mergeCatalogProducts(Array.isArray(store?.products) && store.products.length ? store.products : fallback.products),
    settings: {
...
function storefrontProducts(products) {
  return dedupeProducts(products.filter(isPurchasableProduct));
}

function isPurchasableProduct(product) {
  return Boolean(product) && product.status !== "Archived" && product.commerceMode !== "external" && product.commerceEligible !== false;
}
...
module.exports = { handleAdminRequest, loadStore, saveStore, updateStripeOrder, confirmedOrders, isPurchasableProduct };
